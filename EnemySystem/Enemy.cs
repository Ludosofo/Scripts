...
public class Enemy : MonoBehaviour
{
    ...
    public int maxLife = 160;
    public int life = 1;
    public float localTimeScale = 1f;
    public List<GameObject> listDrop;

    [Header("Enemy Systems")]
    private EnemyMaterialController emc;
    ...
    protected void Awake(){
        ...
        if (emc == null) {
            emc = gameObject.GetComponent<EnemyMaterialController>();
        }
    ...
    /**
     * TakeDamage es una función que toma el damage y el bullet de colision
     * Tras mirarlo, bullet solo lo necesito para obtener el transform.position
     * Esto debería corregirlo
     * **/
    public void TakeDamage(int damage, Vector3 bulletTransformPosition)
    {
        GameObject t = spawner.SpawnDamageText();
        DamageText dt = t.GetComponent<DamageText>();
        dt.SetText(damage);
        dt.UpdatePositionInCanvas(bulletTransformPosition);
    ...
    private void SetSpawner(){
        if (GameObject.Find("PoolingSystem").GetComponent<Spawner>() != null)
        {
            spawner = GameObject.Find("PoolingSystem").GetComponent<Spawner>();
        }
        else {
            Debug.LogError("SetSpawner no consigue acceder a PoolingSystem");
        }
    }
}
